...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    try {
      ...
      // Blob doesn't exist, continue to generation
      console.log('No cached OG image found, generating new one')
    }
...
async function generateFallbackImage(type: string, data: any): Promise<Buffer> {
  try {
    ...
    // Create 1200x630 image with clean light background
    ...
    // Set background color (light gray, matching blog theme)
    for (let y = 0; y < 630; y++) {
      for (let x = 0; x < 1200; x++) {
        image.setPixelColor(0xf9fafbff, x, y)
      }
    }
    ...
    console.error('Jimp fallback failed:', error)
